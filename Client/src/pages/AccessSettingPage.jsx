...
const PermissionChecklist = ({ permissions, selectedIds, onToggle }) => {
  ...
  const toggleGroup = (groupItems) => {
    const allSelected = groupItems.every((permission) =>
    ...
    <div className="grid grid-cols-1 gap-4 max-h-72 overflow-y-auto pr-1 sm:grid-cols-2">
      {groups.map(({ groupKey, label, items }) => {
        ...
        return (
          <div
            key={groupKey}
            className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 p-3"
          >
...
const AccessSettingPage = () => {
  ...
  const fetchPermissions = async () => {
    try {
      ...
      // Don't set error here - let the page continue even if permissions fail
      return false;
    }
  };
  ...
  const fetchRoles = async () => {
    try {
      ...
      // Don't set error here - let the page continue even if roles fail
      return false;
    }
  };
  ...
  const fetchAll = async () => {
    ...
    setError(null);
    try {
      const [permSuccess, rolesSuccess] = await Promise.all([
        fetchPermissions(),
        fetchRoles(),
      ]);

      // Only show error if BOTH fail
      if (!permSuccess && !rolesSuccess) {
        setError(
          "Failed to load access settings. Please check your permissions.",
        );
      }
    } catch (err) {
      console.error("Fetch access settings error:", err);
      setError(
        err.response?.data?.message || "Failed to load access settings.",
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const handleRefresh = (event) => {
      if (event.detail?.pathname && event.detail.pathname !== location.pathname)
        return;
      if (location.pathname !== "/admin/access-settings") return;
      fetchAll();
    };
  ...
  const saveAssignPerm = async () => {
    ...
    const toRemove = existingIds.filter(
      (id) => !assignPermissionIds.includes(id),
    );
  ...
  const hasRolesManage = hasPermission(PERMISSIONS.ROLES_MANAGE);
  const hasPermissionsManage = hasPermission(PERMISSIONS.PERMISSIONS_MANAGE);
  
  const canAssignPermissions =
    hasPermission(PERMISSIONS.PERMISSIONS_ASSIGN) || isSuperAdmin;
  const canUpdatePermission = hasPermission(PERMISSIONS.PERMISSIONS_UPDATE);
  ...
  const canViewRolesTab = hasRolesManage || isSuperAdmin;
  const canViewPermissionsTab = hasPermissionsManage || isSuperAdmin;

  // Filter tabs based on user permissions
  const availableTabs = tabs.filter((tab) => {
    if (tab.id === "roles") return canViewRolesTab;
    if (tab.id === "permissions") return canViewPermissionsTab;
    return true;
  });

  // If activeTab is not available, switch to first available tab
  useEffect(() => {
    if (activeTab === "roles" && !canViewRolesTab) {
      setActiveTab("permissions");
    } else if (activeTab === "permissions" && !canViewPermissionsTab) {
      setActiveTab("roles");
    }
  }, [canViewRolesTab, canViewPermissionsTab, activeTab]);
      ...
      {menuOpen && openMenuType === "role" && (
        <>
          ...
          >
            ...
            {!canAssignPermissions && !canEditRole && !canDeleteRole && (
              <div className="px-4 py-3 text-sm text-slate-500 dark:text-slate-400">
                No permission
              </div>
            )}
