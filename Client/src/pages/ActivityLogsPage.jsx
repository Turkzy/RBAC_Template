...
import { LogIn, LogOut, Settings, Calendar, ChevronDown, ChevronLeft, ChevronRight, KeyRound, Activity, PenSquare, Trash2, Search, Power, UserX, PlusCircle } from "lucide-react";
...
const ActivityLogsPage = () => {
  ...
  const fetchCounts = async () => {
    try {
      const totalRes = await api.get("/activity-logs", { params: { page: 1, limit: 1 } });
      const loginsRes = await api.get("/activity-logs", { params: { page: 1, limit: 1, action: "login" } });
      const createsRes = await api.get("/activity-logs", { params: { page: 1, limit: 1, action: "create" } });
      const updatesRes = await api.get("/activity-logs", { params: { page: 1, limit: 1, action: "update" } });
      const deletesRes = await api.get("/activity-logs", { params: { page: 1, limit: 1, action: "delete" } });
      setCounts({
    ...
    <div className="rounded-lg p-6 select-none">
      <div className="mb-6 flex flex-col gap-4 sm:gap-3 md:flex-row md:items-start md:justify-between">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
          <button
            onClick={() => { fetchLogs(1); fetchCounts(); }}
