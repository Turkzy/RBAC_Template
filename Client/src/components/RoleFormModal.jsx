...
const RoleFormModal = ({ mode, role, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (isEdit) {
        response = await api.put(endpoints.rbac.roles.update(role.id), { name: formData.name });
      } else {
        response = await api.post(endpoints.rbac.roles.create, { name: formData.name });
      }

      SweetAlert.toast.success(isEdit ? "Role updated" : "Role created");
      onSuccess(response.data.role || response.data, mode);
      onClose();
    } catch (err) {
